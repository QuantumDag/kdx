...
const {RPC} = require('@quantumdag_/grpc-node');
...
//TODO
window.PWA_MODULES={};
window.PWA_MODULES["@quantumdag_/wallet-pwa"] = "N/A";
window.PWA_MODULES["@aspectron/flow-ux"] = "N/A";
window.PWA_MODULES["@quantumdag_/ux"] = "N/A";
window.PWA_MODULES["@quantumdag_/grpc-web"] = "N/A";
window.PWA_MODULES["@quantumdag_/wallet"] = "N/A";
window.PWA_MODULES["@quantumdag_/grpc"] = "N/A";
window.PWA_MODULES["@quantumdag_/core-lib"] = "N/A";

class KDXApp extends FlowApp{
	render(){
		let walletMeta = {"generator":"KDX"}
		let list = [
			['Qtm','MIT','Copyright (c) 2020 Qtm Developers'],
...
//			['Mosquitto','EDL-V10 EPL-V10','Copyright (c) 2007, Eclipse Foundation, Inc. and its licensors'],
		...
		let donationAddresses = [
			["Devfund donations:", "qtm:precqv0krj3r6uyyfa36ga7s0u9jct0v4wg8ctsfde2gkrsgwgw8jgxfzfc98"],
			["Mining address:", "qtm:pzhh76qc82wzduvsrd9xh4zde9qhp0xc8rl7qu2mvl2e42uvdqt75zrcgpm00"],
			//["KDX/WebWallet donations:", "qtm:qrncjga8hej9q59q85ge5js6m4y97el6ahp3m87hyzqdtaq6pf0v7xek7x900"],
		]
		return html`
		<flow-caption-bar
			logo="/resources/images/qtm-logo-light-bg.png">KDX</flow-caption-bar>
		<tab-content for="home">
			...
			<div id='qtm-resources'>

				<flow-expandable>
					<div slot="title" is="i18n-div" caption>QTM RESOURCES</div>
					<ul style="font-size: 12px;">
						<li>
							<flow-shell-link href="https://docs.qtm.pa/qtm/about-qtm/get-started">
								<flow-i18n>Documentation</flow-i18n>
							</flow-shell-link>
						</li>
						<li><flow-shell-link
							href="https://github.com/QuantumDag/"><flow-i18n>GitHub</flow-i18n></flow-shell-link></li>
						<li><flow-shell-link
			...
			<flow-expandable class="donation-addresses" no-info _icon="fal:donate">
				...
				<p is="i18n-p">
					if you wish to further the development of the qtm ecosystem, we accept qtm donations at the following addresses:
				</p>
				${
			...
			<flow-form-control icon="fal:copyright">
				<flow-i18n>KDX &amp; Qtm Copyright (c) 2020 Qtm Developers<br/>
		...
		<tab-content for="settings">
			<flow-form-control icon="fal:database">
				...
				<p slot="info" is="i18n-p">
					...
					In default configuration this includes Qtmd blockchain data and Qtmrov API database.
			...
			<flow-form-control icon="fal:cube" class="advanced-tool"
				...
				<p slot="info" is="i18n-p">
					The Enable Mining option starts / stops all configured Quantumdagminer instances.
				</p>
			</flow-form-control>
			...
			<flow-form-control icon="fal:database">
				...
				<p slot="info" is="i18n-p">
					It will delete datadir (Data Folder) and restart qtmd node to re-sync
				</p>
			</flow-form-control>
			<flow-form-control icon="fal:database">
				...
				<p slot="info" is="i18n-p">
					Start the qtmd node without enabling the '--utxoindex' parameter until the synchronization is complete, re-enable this parameter. (Can be used to fix inaccurate balance display)
				</p>
			</flow-form-control>
			<div style="height:192px;"></div>
		</tab-content>
		<tab-content for="wallet" class="wallet" data-active-display="flex">
			<qtm-wallet .walletMeta='${walletMeta}' hideNetwork hidefaucet 
				_hideQRScanner hideopenwalletlogo></qtm-wallet>
		</tab-content>
...
//		this.checkForUpdates().then();
	...
	async initManager(){
		...
		manager.on("task-start", (daemon)=>{
			...
			this.refreshApps();
			const {wallet} = this;
			if(!wallet || daemon.task.type!='qtmd' || !this.rpcDisconnect){
				return
			}
		...
		manager.on("before-interrupt", ({daemon, interrupt})=>{
			...
			const {wallet} = this;
			if(!wallet || daemon.task.type!='qtmd')
				return
		...
		manager.on('sync-status', (data) => {
			//console.log("sync-status:data", data);
			let wallet = this.wallet || this.qS('qtm-wallet');
	...
	async initWallet() {
		...
		let settings = await this.get_default_local_qtmd_settings();
	...
	async get_default_local_qtmd_settings() {
		
		let {config:daemons} = await this.get("get-modules-config");
		console.log("############### DAEMONS", daemons);
		let qtmd = Object.entries(daemons).map(([k,v]) => { 
			const { args } = v;
			const [type, ident] = k.split(':');
			return { type, ident, args};
		}).filter(o=>o.type=='qtmd').shift();

		if(!qtmd)
			return null;//{network:"qtm", port:55110};//{network:"qtmtest", port:55110};
	...
	setTheme(theme){
		...
		if(this.caption)
			this.caption.logo = `/resources/images/qtm-logo-${theme}-bg.png`
		...
		document.body.dispatchEvent(new CustomEvent("flow-theme-changed"));
		this.querySelector("qtm-wallet")?.requestUpdate("theme", null)
	}
	initCaption(){
		...
		this.caption.logo = `/resources/images/qtm-logo-${this.theme||"light"}-bg.png`;
	...
	async initTemplates() {
		...
		if(!this.tpl_template) {
			...
			let miner = Object.keys(config.modules).filter(v=>/^(qtm|gpu)miner/.test(v));
			if(!miner.length)
...
Useful resources:
- Qtm Documentation: https://github.com/QuantumDag/docs 
- Qtm Discord: https://discord.gg/vMT39xB
- Qtm GitHub: https://github.com/QuantumDag/
...
${changelogContent}`;
	...
	async initDaemons(daemons){
		...
		let {params} = this.getModuleArgs('qtmd:', daemons);
