...
class App extends FlowApp{
	...
	*/
	...
	async removeDataDir(){
		try {
			let datadir2path = path.join(this.dataFolder, "qtmd-kd0", "quantumdag-mainnet", "datadir2");
	...
	*/
	...
	getDefaultConfig(){
		...
		Object.entries(config.modules).forEach(([k,v]) => {
			...
			if(['qtmd','qtmrovd','qtmrovsyncd','quantumdagminer'].includes(type)) {
	...
	setSkipUTXOIndex(skipUTXOIndex){
		...
		return this.setModuleArgs("qtmd:", {}, {"skip-utxoindex":skipUTXOIndex})
	}
	getSkipUTXOIndex(){
		let {args, params} = this.getModuleArgs("qtmd:");
	...
	setUpnpIfMissing(upnpEnabled) {
		if (this.config?.modules) {
			Object.keys(this.config.modules).forEach((k) =>{
				if (k.startsWith('qtmd:') && !('upnpEnabled' in this.config.modules[k])) {
