...
*/
    ...
    // getQtmrovTasks() {
    //     return this.manager.tasks.filter(t=>t.type=="qtmrovd");
    // }

    // getQtmrovsyncTasks() {
    //     return this.manager.tasks.filter(t=>t.type=="qtmrovsyncd");
    // }

    // getQtmrovAddress() {
    //     let cfg = this.getQtmrovTasks().shift();
    //     if(!cfg)
    //         return Promise.reject(`qtmrov is not instantiated`);
    //     if(!cfg.args)
    //         return Promise.reject(`qtmrov configuration has no args object`);
    //     if(!cfg.args.listen)
    //         return Promise.reject(`qtmrov configuration arguments have no 'listen' property`);
    ...
    async native_wallet(...args) {
        ...
        let walletBin = this.manager.getDeployableBinary('quantumdagwallet');
        ...
        switch(cmd) {
            ...
            case 'balance': {
                //let qtmrov = await this.getQtmrovAddress();
                // let [address] = args;
                // if(!address)
                //     return Promise.reject('usage: wallet balance <address>');
                //this.log(walletBin,'balance',`/qtmrov-address:${qtmrov}`,`/address:${address}`);
                //return this.spawn(walletBin,['balance',`/qtmrov-address:${qtmrov}`,`/address:${address}`]);
            ...
            //     return this.spawn(walletBin,['send',`--qtmrov-address=${qtmrov}`,`--private-key=${private_key}`,`--to-address=${to_address}`,`--send-amount=${amount}`]);
            // } break;
            // case 'create': {
...
/*
    async migrate(cmd) {
        ...
        const qtmrovsyncdBin = path.join(this.binFolder,'qtmrovsyncd'+(process.platform == 'win32' ? '.exe' : ''));
        if(!fs.existsSync(qtmrovsyncdBin)) {
            this.log(`error: unable to locate ${qtmrovBin}`.red);
            return Promise.reject(`error: qtmrov executable not found`);
        }

        let tasks = this.getQtmrovsyncTasks();
        if(!tasks.length)
            return Promise.reject(`no qtmrovsync tasks detected`);
        ...
        // await utils.spawn(this.getBinary('qtmrovsyncd'), migrateArgs, {
        ...
        while(argsList.length) {
            ...
            await this.spawn(qtmrovsyncdBin,['--migrate', ...args], { 
...
help(term) {
    switch(term) {
        default: {
            ...
            //migrate - upgrades Qtmrov database schema\r
            this.terminal.write(`
native-wallet <command> - run native Qtm CLI wallet command, where command is:\r
\r
...
\r
rpc [command] - run RPC command against the default Qtmd instance\r
\r
    ...
    build(...args) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            this.spawn(process.argv[0], [path.join(this.appFolder,'build','build.js'), ...args], {
                ...
                let target = this.manager.getDeployableBinary('qtmd');
