...
module.exports = class Simulator extends Daemon {
	...
	async start() {
		...
		let defaults = {
			...
			// dbuser: 	'qtm', // Database user
			// dbpass: 	'qtm', // Database password
			// dbname: 	'qtmrov', // Database name
...
//		let {config:daemons} = await this.get("get-modules-config");
		...
		if(!args.miningaddr) {
			args.miningaddr = 'qtmtest:qpjmmd8fc20c6mf5zxr6qax0369g0aufyg6aqcel9u';
			//args.miningaddr = 'qtm:qqg767gkznmsuah5xeer9f72lwqu642nqvlmujtpmk';
			//args.miningaddr = 'qtm:qr35ennsep3hxfe7lnz5ee7j5jgmkjswsslwxj42ta';
		}		
		...
		//console.log("++++++++++++++++ QUANTUMDAGMINER VSARGS")

		await this.VSARGS([this.getDeployableBinary('quantumdagminer'), '--version']);

		args = Object.entries(args);

		const flags = utils.args();
		//console.log("++++++++++++++++ QUANTUMDAGMINER FLAGS",flags)

		this.options = {
			args : () => {
				// this.log(`--addresslist=${configFile}`,`--cert=${rpcCertFile}`);
				let _args_ = [
					this.getBinary('quantumdagminer'),
				...
				console.log("---QUANTUMDAGMINER ARGS",a);
			...
			stdout : (data) => { 
				...
				// // console.log("quantumdagminer match:", match);
		...
		//console.log("QUANTUMDAGMINER STATUS:",this.manager.enableMining,flags.mine);
		if(this.manager.enableMining || flags.mine) {
			dpc(0, () => {
				//console.log("----------------- QUANTUMDAGMINER STARTING")
				this.run()
			})
		} else {
			//console.log("----------------- QUANTUMDAGMINER NOT RUNNING")
			this.state = 'disabled';
			dpc(0, () => {
				this.writeToSink('\r\n\r\nQuantumdagminer is disabled. Please use Enable Mining option to start it.\r\n\r\n');
			})
		}
