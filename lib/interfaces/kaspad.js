...
const {Client:QtmGRPC} = require('@quantumdag_/grpc-node');
const pkg = require("../../package");
const natUpnp = require("@runonflux/nat-upnp");

module.exports = class Qtmd extends Daemon {
	...
	start() {
		...
		return new Promise(async (resolve,reject) => {
			...
			if (!("uacomment" in args)) {
				...
				this.writeToSink(`QTMD: Adding uacomment=${uacomment} to arguments\n`);
			...
			if (this.task.conf['upnpEnabled'] && args['listen'] && args['listen'].indexOf(":") !== -1) {
				...
				if (externalip) {
					if (!("externalip" in args)) {
						this.writeToSink(`QTMD: Adding externalip=${externalip} to arguments\n`);
					...
					if (publicPort) {
						...
						this.writeToSink(`QTMD: Changing externalip from ${args['externalip']} to ${externalip}:${publicPort} in arguments\n`);
						args['externalip'] = `${externalip}:${publicPort}`;
						if (publicPort !== nodePort) {
							this.writeToSink(`QTMD: Changing listener port from ${nodePort} to ${publicPort} in arguments\n`);
							args['listen'] = `${args['listen'].split(':')[0]}:${publicPort}`;
						}
						if (!("maxinpeers" in args)) {
							this.writeToSink(`QTMD: Adding maxinpeers=${this.upnpMaxInPeers} to arguments\n`);
			...
			const configFile = path.join(this.folder, 'qtmd.conf');
			...
			await this.VSARGS([this.getDeployableBinary('qtmd'),'--version']);

			this.run({
				args : async () => {
					let _args_ = [
 						this.getDeployableBinary('qtmd')
					];
			...
			resolve();
			// mutes all qtmd processes by default
			this.mute = true;
		})
	}
		...
		return new Promise(async (resolve, reject) => {
	        ...
	        try {
				...
				this.data = this.sourcesToData(sources);
	             ...
	             console.log('QTMD monitor:'.bold,ex);
	            return reject(ex);
	        }
